    ...
    const clickHandler = (type: TypeInterface) => {
        if (type.id === productStore.selectedType?.id) {
            ...
            getType(type.id).then((type) => {
                ...
                if (productStore.selectedBrand && !Array.from(productStore.brands, (brand) => brand.id).includes(productStore.selectedBrand?.id)) {
    ...
    return (
        <ListGroup className="types_list mb-3">
            {productStore.types.map((type) => (
                <ListGroup.Item action active={productStore.selectedType ? type.id === productStore.selectedType.id : false} key={type.id} onClick={() => clickHandler(type)}>
                    {type.name}
                </ListGroup.Item>
            ))}
        </ListGroup>
