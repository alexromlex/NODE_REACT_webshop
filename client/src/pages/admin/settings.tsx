...
const SettingsPage = () => {
    ...
    const tynimceItinsettings = {
        ...
        menubar: true,
        license_key: 'gpl',
        plugins: ['accordion', 'advlist', 'anchor', 'autolink', 'autoresize', 'charmap', 'code', 'lists', 'link', 'image', 'preview', 'anchor', 'searchreplace', 'visualblocks', 'code', 'fullscreen', 'insertdatetime', 'table'],
        toolbar: 'undo redo | formatselect | bold italic backcolor | alignleft aligncenter alignright alignjustify  | table | bullist numlist outdent indent | removeformat',
    ...
    const SaveHandler = () => {
        ...
        for (const [key, val] of Object.entries(settings)) {
            ...
            if (val !== adminStore.settings.key) {
                toUpdate.push({ name: key, value: val });
            }
        }
        if (settings.privacy_policy !== privacy_policyRef.current?.value) toUpdate.push({ name: 'privacy_policy', value: privacy_policyRef.current?.getContent() });
        if (settings.general_terms !== general_termsRef.current?.value) toUpdate.push({ name: 'general_terms', value: general_termsRef.current?.getContent() });

        adminStore.updateSettings(toUpdate);
    };
    useEffect(() => {
            ...
            {Object.keys(settings).length === 0 ? (
                ...
                <>
                    ...
                    <Card className="p-0 m-0 mb-3">
                        ...
                        <Card.Body>
                            <Editor tinymceScriptSrc={'/tinymce/tinymce.min.js'} onInit={(evt, editor) => (privacy_policyRef.current = editor)} initialValue={settings?.privacy_policy} init={tynimceItinsettings} />
                        </Card.Body>
                    </Card>
                    <Card className="p-0 m-0 mb-3">
                        <Card.Header className="fw-bold">General Terms and Conditions</Card.Header>
                        <Card.Body>
                            <Editor tinymceScriptSrc={'/tinymce/tinymce.min.js'} onInit={(evt, editor) => (general_termsRef.current = editor)} initialValue={settings?.general_terms} init={tynimceItinsettings} />
                        </Card.Body>
                    </Card>
